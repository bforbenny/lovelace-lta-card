class LTACard extends HTMLElement {
  set hass(hass) {
    ...
    var tmpBusArr = "";
    var tmpBusName = "";
    var threshold = this.config.threshold ? this.config.threshold : 0;
    var class_late = "";
    var bus_type = "";
    ...
    this.config.group.forEach(function (item, index) {
      ...
      item["entities"].forEach(function (item, index) {
        if (hass.states[item["entity"]]) {
          class_late =
            hass.states[item["entity"]].state < threshold ? "bus_late" : "";
          bus_type =
            hass.states[item["entity"]].attributes.type == "DD"
          ...
          tmpBusArr =
            tmpBusArr +
            `
                      <div class="bus_time"><div class="bus_time_value ${class_late}">${
              hass.states[item["entity"]].state
            }</div> ${
              hass.states[item["entity"]].attributes.unit_of_measurement
            } <ha-icon icon="${bus_type}"></ha-icon></div>
                    `;
        }
      });
